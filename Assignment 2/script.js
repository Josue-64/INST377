function quoteOfTheDay() {
    fetch('https://zenquotes.io/api/random')
        ...
        .then(data => {
            document.getElementById("quoteText").innerHTML = `<p>"${data[0].q}"</p><p>- ${data[0].a}</p>`;
        });
}
...
async function getRedditStocks() {
    const resultJson = await fetch('https://apewisdom.io/api/v1.0/filter/all-stocks/page/1')
        .then(result => result.json());
...
async function getDogBreeds() {
    ...
    const breeds = Object.keys(breedList.message).sort(() => Math.random() - 0.5).slice(0, 10);
    const breedButtonsDiv = document.getElementById("breedButtons");

    const allDogApiBreeds = await fetch('https://dogapi.dog/api/v2/breeds')
        .then(result => result.json());
    ...
    breeds.forEach((breedName, index) => {
        ...
        button.onclick = () => showBreedInfo(allDogApiBreeds.data[index]);
...
function showBreedInfo(breed) {
    ...
    document.getElementById("breedMinLife").textContent = `Min Life: ${breed.attributes.life.min} years`;
    document.getElementById("breedMaxLife").textContent = `Max Life: ${breed.attributes.life.max} years`;
